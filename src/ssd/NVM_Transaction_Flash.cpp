...
{
	NVM_Transaction_Flash::NVM_Transaction_Flash(Transaction_Source_Type source, TransactionType type, stream_id_type stream_id,
	...
	NVM_Transaction_Flash::NVM_Transaction_Flash(Transaction_Source_Type source, TransactionType type, stream_id_type stream_id,
		...
		NVM_Transaction(stream_id, user_request),
		Source(source), Type(type), Address(address),
		Issue_time(Simulator->Time()), STAT_execution_time(INVALID_TIME),
		STAT_transfer_time(INVALID_TIME), Physical_address_determined(false)
	{
	}
}
