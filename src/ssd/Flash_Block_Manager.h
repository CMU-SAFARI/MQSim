...
namespace SSD_Components
{
	...
	public:
		...
		void Allocate_block_and_page_in_plane_for_user_write(const stream_id_type streamID, NVM::FlashMemory::Physical_Page_Address& address);
		void Allocate_block_and_page_in_plane_for_translation_write(const stream_id_type streamID, NVM::FlashMemory::Physical_Page_Address& address);
		...
		void GetWearlevelingBlocks(BlockPoolSlotType*& hotBlock, BlockPoolSlotType*& coldBlock);
		unsigned int Get_pool_size(const NVM::FlashMemory::Physical_Page_Address& plane_address);
	private:
		PlaneBookKeepingType**** planeManager;//Keeps track of plane block usage information
