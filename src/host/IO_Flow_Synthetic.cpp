...
namespace Host_Components
{
	...
	IO_Flow_Synthetic::IO_Flow_Synthetic(const sim_object_id_type& name, LSA_type start_lsa_on_device, LSA_type end_lsa_on_device,  uint16_t io_queue_id,
		uint16_t nvme_submission_queue_size, uint16_t nvme_completion_queue_size, IO_Flow_Priority_Class priority_class,
		double read_ratio, Address_Distribution_Type address_distribution,
		...
		IO_Flow_Base(name, start_lsa_on_device, end_lsa_on_device, io_queue_id, nvme_submission_queue_size, nvme_completion_queue_size, priority_class, stop_time, total_req_count, SSD_device_type, pcie_root_complex), read_ratio(read_ratio), address_distribution(address_distribution),
		hot_address_ratio(hot_address_ratio),
		...
		seed(seed)
	{
		...
		if (request_size_distribution == Request_Size_Distribution_Type::NORMAL)
	...
	Host_IO_Reqeust* IO_Flow_Synthetic::Generate_next_request()
	{
		...
		{
		case Address_Distribution_Type::STREAMING:
			...
			break;
		case Address_Distribution_Type::HOTCOLD_RANDOM:
			...
			break;
		case Address_Distribution_Type::UNIFORM_RANDOM:
	...
	{
		if (address_distribution == Address_Distribution_Type::STREAMING)
