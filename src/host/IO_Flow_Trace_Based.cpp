...
{
	IO_Flow_Trace_Based::IO_Flow_Trace_Based(const sim_object_id_type& name, LHA_type start_lsa_on_device, LHA_type end_lsa_on_device, uint16_t io_queue_id,
		...
		IO_Flow_Base(name, start_lsa_on_device, end_lsa_on_device, io_queue_id, nvme_submission_queue_size, nvme_completion_queue_size, priority_class, 0, initial_occupancy_ratio, 0, SSD_device_type, pcie_root_complex, enabled_logging, logging_period, logging_file_path),
